...
. When deployed alongside IBM Storage Fusion, the ODF operators are installed from the *IBM Storage Fusion Data Foundation* catalog (`isf-data-foundation-catalog`) rather than the default `redhat-operators` catalog
...
The `replica: 3` field means Ceph creates three copies of each OSD across different nodes for resiliency. This triggers Rook to deploy the Ceph cluster: MON daemons, OSD daemons (one per storage device per replica), and MDS daemons for CephFS.

=== Step 3: IBM Storage Fusion Operator

. The IBM Storage Fusion operator is installed from the *IBM operator catalog* (`ibm-operator-catalog`) on the `v2.0` channel
