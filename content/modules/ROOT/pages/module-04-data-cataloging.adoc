...
+
. Create a policy to tag files that may contain personal information. Try asking Lightspeed:
+
* "Create an AUTOTAG policy called tag-sensitive-files that filters on filename LIKE '%customer%', runs NOW, and sets SENSITIVITY to pii-likely"
+
If the policy creation fails (same `tags` limitation as Step 10), use the terminal fallback:
+
[source,bash,role=execute,subs="attributes+"]
----
MCP_HOST=$(oc -n {dcs_namespace} get route dcs-mcp-route -o jsonpath='{.spec.host}')
...
echo "Creating policy tag-sensitive-files..."
curl -sk "https://$MCP_HOST/mcp/http" \
  -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
  -H "Mcp-Session-Id: $SID" \
  -d "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"tools/call\",\"params\":{\"name\":\"dcs_create_policy\",\"arguments\":{\"policyName\":\"tag-sensitive-files\",\"polFilter\":\"filename LIKE '%customer%'\",\"schedule\":\"NOW\",\"tags\":{\"SENSITIVITY\":\"pii-likely\"}}}}" 2>/dev/null | grep -o '"text":"[^"]*"' | cut -d'"' -f4
----
+
