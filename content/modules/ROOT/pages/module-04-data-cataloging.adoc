...
=== Step 8b: Authenticate with DCS

Before querying the catalog, Lightspeed needs DCS credentials. In the chat panel, type:

----
Connect to DCS using username sdadmin and the cluster URL https://api.ocp.{cluster_domain}:6443
----

When Lightspeed asks for the password, retrieve it and provide it:
...
Lightspeed will call the `dcs_set_credentials` MCP tool to authenticate. You should see a confirmation message like "Token generated successfully for sdadmin".

[TIP]
====
If Lightspeed does not recognize the DCS tools, verify that the MCP feature is enabled by checking for the chat panel's tool indicator. The `OLSConfig` must include `featureGates: ["MCPServer"]` and the `mcpServers` entry pointing to the DCS MCP route.
====
