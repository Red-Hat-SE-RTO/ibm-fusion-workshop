...
+
. Create an auto-tagging policy. Describe each parameter on its own line so the LLM maps them correctly -- especially `tags`, which must be passed as a JSON object, not a string:
+
----
Call the dcs_create_policy tool with these parameters:
  policyName = classify-pdfs
  polFilter = filetype='pdf'
  schedule = NOW
  tags = map with key DOC_CATEGORY and value reference-guide
----
+
[TIP]
====
*Why so specific?* The `dcs_create_policy` tool has strict requirements:

* `tags` must be a JSON object (map), not a string. Writing it as `tags: {"DOC_CATEGORY": "reference-guide"}` in the prompt confuses some LLMs -- they drop the parameter or pass it as a string. Phrasing it as _"map with key X and value Y"_ reliably produces the correct JSON object.
* Tag names must be *UPPERCASE* -- `DOC_CATEGORY` works, `doc_category` fails with "Invalid tag in policy action params."
* `polFilter` uses SQL-like syntax -- `filetype='pdf'` or `filename LIKE '%customer%'`
====
+
...
[IMPORTANT]
====
The LLM can *propose and apply* tags programmatically through MCP tool calls. This transforms catalog maintenance from a manual, transactional process into an intelligent, conversational workflow. Each action calls a specific MCP tool -- you can watch the tool invocations in the Lightspeed chat as they happen.
====

=== Step 11: Security Risk Assessment

This exercise mirrors the use case described in the https://community.ibm.com/community/user/blogs/paul-llamas-virgen/2026/03/06/exploring-your-fusion-data-catalog-with-nlp[IBM Community article]. You will create a security classification tag, apply it to potentially sensitive files, and then generate a posture summary.

. First, create a security classification tag:
+
* "Create a tag called SENSITIVITY with type VARCHAR(256)"
+
. Create a policy to tag files that may contain personal information:
+
----
Call the dcs_create_policy tool with these parameters:
  policyName = tag-sensitive-files
  polFilter = filename LIKE '%customer%'
  schedule = NOW
  tags = map with key SENSITIVITY and value pii-likely
----
+
. Check what was tagged:
+
* "Show me all files that have a SENSITIVITY tag set. Include the filename, filetype, and sensitivity value."
+
You should see `customer-directory.csv` tagged as `pii-likely`.
+
. Generate a security posture summary:
+
* "How many files in the catalog are classified (have any tag set like DOC_CATEGORY or SENSITIVITY) versus unclassified? Give me a count of each."
+
The result should show 4 classified files (3 PDFs with DOC_CATEGORY + 1 CSV with SENSITIVITY) out of 25 total.

[NOTE]
====
This is a metadata-based risk assessment -- the AI is classifying files by name patterns and file types, not by reading file contents. In a production environment with more data sources and richer metadata, MCP-driven classification scales to thousands of files in minutes, replacing days of manual SQL queries and spreadsheet analysis.
====
