...
=== Step 11: Verify Policy Results and Assess Coverage

Now that the `classify-pdfs` policy has executed, use Lightspeed to verify the results and assess classification coverage.

. Verify which files were tagged by the policy. Give the LLM the SQL directly -- custom tag names like `DOC_CATEGORY` are valid column names in the METAOCEAN table, but the tool schema only advertises generic `tag1-tag16`, so the LLM won't discover them on its own:
+
* "Run this query: SELECT FILENAME, DOC_CATEGORY FROM METAOCEAN WHERE FILETYPE = 'pdf'"
+
You should see all 3 PDFs tagged with `reference-guide`.
+
. Assess classification coverage:
+
* "Run this query: SELECT CASE WHEN DOC_CATEGORY IS NOT NULL AND DOC_CATEGORY <> '' THEN 'classified' ELSE 'unclassified' END AS STATUS, COUNT(*) AS CNT FROM METAOCEAN GROUP BY STATUS"
+
You should see 3 classified and 22 unclassified out of 25 total -- highlighting the gap.
+
. Identify files that may need attention based on their names:
+
* "Which files have names containing 'customer' or 'audit'? Show the filename, filetype, and size."
+
Lightspeed will find `customer-directory.csv` and `api-audit-2026-03-31.log` -- files that could contain PII or sensitive operations data, but are not yet classified.
+
. Explore further:
+
* "What is the total storage consumed by each file type?"
* "Show me all YAML files under the manifests/ directory -- do any look like they contain credentials or secrets?"
