...
. First, ask Lightspeed to analyze your data:
+
...
+
. Create an auto-tagging policy. Be explicit about the tags parameter format (the tool expects a map):
+
* "Create a policy named classify-pdfs with polFilter filetype='pdf', schedule NOW, and tags \{"doc_category": "reference-guide"}"
+
[TIP]
====
If the policy creation fails with _"'tags' parameter is required and must be a map"_, rephrase with the explicit JSON format: `tags: {"doc_category": "reference-guide"}`. The `dcs_create_policy` tool expects `tags` as a key-value object, and some LLMs need the exact structure spelled out.
====
+
