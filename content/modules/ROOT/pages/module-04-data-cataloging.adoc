...
+
. Create an auto-tagging policy. You must be very explicit about the parameter format -- spell out each field including the JSON `tags` map with *UPPERCASE* tag names:
+
* "Use the dcs_create_policy tool with policyName: classify-pdfs, polFilter: filetype='pdf', schedule: NOW, tags: \{"DOC_CATEGORY": "reference-guide"}"
+
[TIP]
====
*Why so explicit?* The `dcs_create_policy` tool has strict requirements:

* `tags` must be a JSON object (map), not a string -- e.g. `{"DOC_CATEGORY": "reference-guide"}`
...
If Lightspeed gets the format wrong, paste the exact tool call format above.
====
+
...
+
* "Use the dcs_create_policy tool with policyName: tag-sensitive-files, polFilter: filename LIKE '%customer%', schedule: NOW, tags: \{"SENSITIVITY": "pii-likely"}"
+
