...
[WARNING]
====
Do *not* reword or shorten the message. The LLM needs the exact parameter names (`cluster_url`, `username`, `password`) and values to call the tool correctly. If the tool fails with "x-auth-token not found", double-check that the `cluster_url` matches `oc whoami --show-server` exactly.
====
